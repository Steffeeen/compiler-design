...
private class Lowerer(private val irGraph: IrGraph) {
    ...
    fun lower(): AsmIr.Function {
        ...
        for (returnNode in irGraph.endNode.returnNodes) {
            ...
            with(block) {
                generateSideEffectNode(returnNode.sideEffect, returnNode)
            }
        }
    ...
    private fun generateSideEffectNode(sideEffectNode: IrNode.SideEffectNode, previousSideEffectNode: IrNode.SideEffectNode) {
        ...
        when (sideEffectNode) {
            ...
            is IrNode.ReturnNode -> generateSideEffectNode(sideEffectNode.sideEffect, sideEffectNode)
            is IrNode.DivNode -> {
                generateDataNode(sideEffectNode, register(sideEffectNode))
                generateSideEffectNode(sideEffectNode.sideEffect, sideEffectNode)
            }
            ...
            is IrNode.SideEffectPhiNode -> {
                ...
                with(block1) {
                    generateSideEffectNode(sideEffectNode.first, previousSideEffectNode)
                }

                val block2 = controlsToBlock[sideEffectNode.secondControl]!!
                with(block2) {
                    generateSideEffectNode(sideEffectNode.second, previousSideEffectNode)
                }
            }
