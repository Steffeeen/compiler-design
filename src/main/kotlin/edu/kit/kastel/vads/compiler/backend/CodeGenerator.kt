...
private enum class Instruction {
    MOV, PUSH, POP, ADD, SUB, IMUL, IDIV, RET, CDQ, NEG;
...
fun generateX86Assembly(irGraphs: List<IrGraph>): X86Assembly = X86Assembly(buildString {
    ...
    for (irGraph in irGraphs) {
        generateFunctionPrefix(irGraph.name)
        generateFunction(irGraph.linearize())
        generateFunctionSuffix()
    }
...
private fun StringBuilder.generateFunction(nodes: List<IrNode>) {
...
private fun StringBuilder.generateFunctionSuffix() {
    appendLine()
}